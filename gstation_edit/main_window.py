...
class MainWindow:
    ...
    def init_bank_list_widget(self):
        ...
        if self.bank_list_widget is None:
    ...
    def init_context_menu_widget(self):
        ...
        if self.context_menu_widget is None:
    ...
    def send_parameter_value(self, parameter, program_has_changed=True):
        if program_has_changed and not self.current_program is None:
    ...
    def set_program_has_changed(self, has_changed):
        if self.current_selected_iter is None:
            return
    ...
    def receive_program(self, program):
        ...
        if not self.current_program is None and self.current_program.number == program.number:
            self.current_program = program
            self.set_current_name(program.name)
            self.init_parameters()

    def select_program_from_its_number(self, program_nb):
        if self.current_program is None or \
    ...
    def set_current_program(self, program_nb):
        ...
        if program is None:
            print('Unknown program selection %d out of bounds'%(program_nb))
            return

        if not self.current_program is None:
    ...
    def update_parameter_from_jstation(self, parameter, value, is_cc):
        ...
        if parameter:
            ...
            # FIXME self.current_program availability was not checked before
            if parameter.parameter_nb != -1 and not self.current_program is None:
                self.current_program.change_parameter(parameter.parameter_nb,
                                                      parameter.value)
                self.set_program_has_changed(self.current_program.has_changed)
    ...
    def select_program_from_ui(self, widget):
        ...
        cur_program_nb = int(
            self.bank_list_model.get_value(self.current_selected_iter, 0)
        )
    ...
    def import_buffer(self, sysex_data):
        ...
        if sysex_event is None or not sysex_event.is_valid():
